...
# ylim is a shared c(min, max) range (computed below across all 3 sources)
# applied via coord_cartesian() so all 3 panels stay visually comparable.
build_hdi_width_plot <- function(sweep_results, y_lab, plot_title = NULL, ylim) {
  ...
  # 50 is dropped from the displayed breaks only (data untouched) since it
  # sits too close to 100 to label legibly at this text size.
  sample_sizes <- sort(unique(sweep_results$sample_size))
  sample_sizes <- sample_sizes[sample_sizes != 50]
...
# Shared y-axis range across all 3 sweep_results, computed before building any
# panel, so the 3 plots (separate ggplot objects, not facets) end up on an
# identical hdi_width scale for visual comparability.
hdi_cols <- c("hdi_width_85", "hdi_width_90", "hdi_width_95")

shared_ylim <- range(
  sweep_results_sigmas[hdi_cols],
  sweep_results_phq9[hdi_cols],
  sweep_results_cbcu[hdi_cols],
  na.rm = TRUE
)

p_a <- build_hdi_width_plot(
  sweep_results_sigmas, y_lab = "HDI width (PHQ9 ICC)", ylim = shared_ylim
)

p_b <- build_hdi_width_plot(
  sweep_results_phq9, y_lab = "HDI width (subject ICC)", ylim = shared_ylim
)

p_c <- build_hdi_width_plot(
  sweep_results_cbcu, y_lab = "HDI width (subject ICC)", ylim = shared_ylim
)

#### PLOT KEY_DECAY RECOVERY ####

plot_df <- data.frame(
  true_val      = key_decay_recovery$true_value,
  recovered_val = key_decay_recovery$recovered_value
)
plot_df <- plot_df[complete.cases(plot_df), ]
stopifnot(nrow(plot_df) == nrow(key_decay_recovery))
...
p_key_decay <- ggplot(plot_df, aes(x = true_val, y = recovered_val)) +

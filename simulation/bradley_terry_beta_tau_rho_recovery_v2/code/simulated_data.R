...
# range (bradley_terry_beta_tau_rho.stan, model block), rather than drawn from the priors.
mu_log_beta    <- 0.5
sigma_log_beta <- 0.3
mu_tau         <- 0
sigma_tau      <- 1
...
# true_beta/true_tau/true_rho are joined per subject so the saved CSV carries each
# row's generating parameters alongside its trial data.

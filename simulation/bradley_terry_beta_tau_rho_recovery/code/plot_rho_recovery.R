...
plot_df <- data.frame(
  true_val      = rho_recovery$true_value,
  recovered_val = rho_recovery$recovered_value
)
plot_df <- plot_df[complete.cases(plot_df), ]
stopifnot(nrow(plot_df) == nrow(rho_recovery))
...
p_rho <- ggplot(plot_df, aes(x = true_val, y = recovered_val)) +
  ...
  labs(x = "True rho", y = "Recovered rho")

ggsave(file.path(output_dir, "rho_recovery.pdf"), plot = p_rho, width = 10, height = 8, bg = "white")
ggsave(file.path(output_dir, "rho_recovery.png"), plot = p_rho, width = 10, height = 8, dpi = 300, bg = "white")

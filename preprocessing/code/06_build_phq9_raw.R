...
phq9_results <- collected |>
  ...
  dplyr::select(-participant_id, -session, -time_elapsed) |>
...
phq9_results <- phq9_results |>
  dplyr::mutate(
    ...
    completion_time    = hms::hms(seconds = completion_time / 1000)
  )
...
phq9_results_dictionary <- tibble::tribble(
  ...
  "completion_time",     "hms",       "time from PHQ9 grid presentation to submit, as hms. One row per session (the whole matrix); no item-wise RT. Equal to jsPsych rt / time_to_submit_ms, which start at stimulus onset.",
...
write_data_validation_report(phq9_results, phq9_results_dictionary, "phq9", step = "06")

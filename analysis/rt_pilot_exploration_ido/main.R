...
code_dir      <- file.path(project_root, "analysis", "rt_pilot_exploration_ido", "code")
artifacts_dir <- file.path(project_root, "analysis", "rt_pilot_exploration_ido", "artifacts")
output_dir    <- file.path(project_root, "analysis", "rt_pilot_exploration_ido", "output")

dir.create(artifacts_dir, showWarnings = FALSE, recursive = TRUE)
dir.create(output_dir, showWarnings = FALSE, recursive = TRUE)

# One-off look at Ido's pilot session, read directly from data/collected/
# (not data/processed/) since this pilot run never goes through the real pipeline.
data_path <- file.path(project_root, "data", "collected", "results_ido.csv")
